...
GKSidebarOverride.prototype.addRule = function() {
	...
	if(jQuery('#gk_sidebar_override_option').val() != '' && !founded) {
		$this.rulesList.append('<li data-option="'+jQuery('#gk_sidebar_override_option').val()+'" data-position="'+jQuery('#gk_sidebar_position').val()+'" data-width="'+jQuery('#gk_sidebar_width').val()+'"><p class="option">'+jQuery('#gk_sidebar_override_option').val()+'</p><p class="position">'+jQuery('#gk_sidebar_position').val()+'</p><p class="width">'+jQuery('#gk_sidebar_width').val()+'</p><button class="btn btn-link removeRule"><span class="fa fa-remove"></span></button></li>');
		$this.updateJSON();
...
GKSidebarOverride.prototype.init = function() {
	...
	jQuery('#sidebar_rules_list').click(function(e) {
		if(jQuery(e.target).hasClass('removeRule') || jQuery(e.target).hasClass('fa-remove')) {
			e.preventDefault();
			e.stopPropagation();
...
GKSidebarOverride.prototype.renderRules = function() {
	...
	jQuery(rules).each(function(i, rule) {
		$this.rulesList.append('<li data-option="'+rule.option+'" data-position="'+rule.position+'" data-width="'+rule.width+'"><p class="option">'+rule.option+'</p><p class="position">'+rule.position+'</p><p class="width">'+rule.width+'</p><button class="btn btn-link removeRule"><span class="fa fa-remove"></span></button></li>');
	});
}

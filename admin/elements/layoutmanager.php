	...
	protected function getInput() {
		...
		if($layout != '') {
			...
			$html .= '<div id="lManagerInfo" class="alert alert-info">Layout Manager is available is available only when screen resolution is at least 1024px wide.</div><div class="btn-toolbar" id="lManagerMode">
			  <div class="btn-group">
			    <a class="btn active" id="desktop" href="#"><i class="fa fa-desktop"></i></a>
			    <a class="btn" href="#" id="tablet"><i class="fa fa-tablet"></i></a>
			    <a class="btn" href="#" id="mobile"><i class="fa fa-mobile"></i></a>
			  </div>
			</div>';
			...
			$html .= '<script id="moduleTemplate" type="text/template">
					 <% if (type == "module" && position != "sidebar") { %>
						  <div class="span12">
						  	<div class="span2"><button class="btn btn-small btn-link configuration" data-toggle="popover"><i class="fa fa-cog"></i></button></div>
						 	<div class="span7 title"><span class="label label-<%= type %>"><%= position %></span><span class="badge badge-warning"><%= modules.length %></span></div>	  
						  	<div class="span2 pull-right"><button class="btn btn-small btn-link add-modules pull-right" data-html="true" data-original-title="Add modules" data-toggle="popover"><i class="fa fa-plus"></i> <span>Add</span></button></div></div>
					  <% } else if( position != "sidebar") { %>
					      <div class="span12"><span class="label label-<%= type %>"><%= position %></span></div>
					  <% } else { %>
					  	 <div class="btnWrap">
					  	 <a class="btn btn-link sidebarOverride" id="sidebarOverride"><span class="fa fa-cogs"></span><small>OVERRIDE</small></a>
					  	 </div>
					  	 ...
					  	 <button class="btn btn-small decVal"><i class="fa fa-arrow-down"></i></button><input class="input-small span1 widthValue" id="appendedPrependedDropdownButton" type="text" value="<%= width.substr(4,5) %>"><button class="btn btn-small incVal"><i class="fa fa-arrow-up"></i></button>
					  	 </div></div>
					  <% } %>
					  ...
					  <% if(modules.length) { %>
					    ...
					    <% _.each(modules, function (module, i) {%>
					      <div class="span<%= module.width_desktop%> module" data-desktop="<%= module.width_desktop%>" data-tablet="<%= module.width_tablet%>" data-mobile="<%= module.width_mobile%>" data-order="<%= i %>">
					      		<div>
					      		<button class="btn btn-link btn-mini decWidth"><i class="fa fa-minus"></i></button><input class="width" type="text" value="<%= module.width_desktop%>"><button class="btn btn-link btn-mini incWidth"><i class="fa fa-plus"></i></button><button class="btn btn-link btn-mini removeModule"><i class="fa fa-remove"></i></button>
					      		</div>
					      </div>
			...
			$html .= '</script>';
			$html .= '<div id="popOverContent"><button class="btn btn-small decVal"><i class="fa fa-arrow-down"></i></button><input class="input-small span1 widthValue" id="appendedPrependedDropdownButton" type="text" value="6"><button class="btn btn-small incVal"><i class="fa fa-arrow-up"></i></button><button class="btn btn-small btn-success addModule" type="button">Add</button>
